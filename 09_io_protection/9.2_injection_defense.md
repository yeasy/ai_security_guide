...
> 说明：若阅读环境不支持 Mermaid 渲染，可将分离方式理解为：系统提示作为“指令层”，用户输入作为“数据层”，两者明确分隔后再交给模型。
...
> 说明：若阅读环境不支持 Mermaid 渲染，可将注入检测理解为：先用检测器判断输入是否可疑，再决定交给主模型处理或直接拒绝/净化。
...
> 说明：若阅读环境不支持 Mermaid 渲染，可将间接注入防护理解为：外部数据先提取与过滤，再做来源标记，最后才允许进入上下文。
...
提示注入防御需要多层措施协同工作。没有单一方法能100%防御，关键是增加攻击成本和降低成功概率。
