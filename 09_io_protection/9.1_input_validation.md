...
语义检测的典型架构是：获取输入嵌入 → 通过分类模型得分 → 与阈值对比：

```mermaid
flowchart LR
    A["输入文本"] --> B["嵌入向量"]
    B --> C["分类模型<br/>(Llama Guard 等)"]
    C --> D{"恶意概率 > 阈值?"}
    D --> |是| E["拒绝/标记"]
    D --> |否| F["通过"]
```
...
并不是所有检测到的"可疑内容"都应该直接拒绝——这样会导致高误报率和糟糕的用户体验。更精细的做法是根据检测的"置信度"和"严重性"采取分级的处理策略：
- **高置信度 + 高危机制**（如系统提示泄露企图）：直接拒绝
- **中置信度 + 中等风险**（如可能的注入尝试）：移除可疑部分并继续处理
...
| **净化** | 移除可疑部分，继续处理清洁版本 | 可分离的恶意内容、注入片段 | 中等 |
