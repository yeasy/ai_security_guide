...
这里的“新增”更准确地说是：在多智能体系统里被显著放大、并且需要在系统级重新建模的风险。像信任传递、权限泄露、共享状态污染等问题，前文在 [7.1 节](7.1_agent_risks.md) 已经从总览视角出现；本节的重点是把它们放进 **多智能体拓扑、消息链路和零信任架构** 中重新分析。
...
class MessageAuthenticationLayer:
    ...
    def verify_and_process_message(self, signed_message, sender_public_key):
        ...
        if (datetime.now() - message_time).total_seconds() > 300:  # 5分钟过期
