...
“Rule of Two” 是 Meta 在 2025 年提出的一项重要 AI 智能体安全实践指导。按照 [Meta 原文](https://ai.meta.com/blog/practical-ai-agent-security/)，这一原则的核心不是“所有不可逆操作都必须由两个 Agent 批准”，而是：**在单个 session 中，一个 agent 最多只能同时满足以下三项性质中的两项：处理不可信输入、访问敏感系统/私有数据、改变状态或对外通信；若三项都需要，则至少要加入监督或可靠验证。** 本节将围绕这一更准确的定义来讨论其设计含义。
...
> **先厘清命名**：本节的「双 Agent 复核」是一种**监督网关**实现，不是 Rule of Two 本身。
> 按 7.6.1 引的 Meta 原文，Rule of Two 约束的是**单个 session 内一个 agent 最多同时具备三项性质中的两项**；当三项都需要时，Meta 给出的基线是**人工确认（human-in-the-loop）或其他可靠校验**。用第二个 LLM Agent 去复核是一种更弱的替代——本节 7.6.4「勾结攻击」正是在讲它为什么脆弱。下文的代码与流程图沿用「双 Agent」写法时，请按此理解。
...
class ImprovedDatabaseAgent:
    ...
    # 注意：审计日志（audit_logs）这类取证数据应当只追加、不可删除，
    # 永远不要把它放进删除白名单；这里只允许清理临时导出与过期会话
    ALLOWED_DELETE_TARGETS = {
        "temp_exports": {"owner_id", "created_at"},
        "expired_sessions": {"user_id", "expires_at"},
    }

    def __init__(self):
        self.permission_level = PermissionLevel.MINIMAL

    def build_where_clause(self, table_name, filters):
        """只允许白名单字段参与条件拼装，具体值走参数绑定"""
        allowed_columns = self.ALLOWED_DELETE_TARGETS.get(table_name)
        if not allowed_columns:
            raise PermissionDenied(f"Table is not approved for deletes: {table_name}")

        clauses = []
        params = {}
        for column, value in filters.items():
            if column not in allowed_columns:
                raise PermissionDenied(f"Column is not approved for deletes: {column}")
            param_name = f"p_{column}"
            clauses.append(f"{column} = :{param_name}")
            params[param_name] = value

        if not clauses:
            raise ValueError("Delete filters must not be empty")

        return " AND ".join(clauses), params

    def execute_delete(self, table_name, filters):
        """删除操作的改进流程"""
        ...
        where_clause, params = self.build_where_clause(table_name, filters)

        # Step 2: Dry-run - 统计将要删除的记录。表名和列名只来自白名单，值走参数绑定。
        # 日志只记录脱敏摘要（行数、表名、条件列），不把整条记录写入日志；
        # 完整记录预览仅通过下方 request_human_approval 的 preview 参数交给审批人
        ...
        logger.info(
            f"Will delete {len(affected_records)} records "
            f"from {table_name} (filter: {where_clause})"
        )
...
class AgentRotationDefense:
    ...
    def initialize_agent_pool(self):
        ...
        return [
            ...
            ModelAgent(vendor='Meta', model='current-meta-primary'),
            ModelAgent(vendor='Mistral', model='current-mistral-primary'),
        ]
...
class SafeProcurementAgent:
    ...
    def purchase(self, service, cost, duration):
        ...
        if cost > self.BUDGET_LIMIT:
            ...
            return self.request_human_approval(cost, reason="Exceeds single-purchase budget limit")

        if self.monthly_spent + cost > self.MONTHLY_LIMIT:
            return self.request_human_approval(cost, reason="Exceeds monthly budget limit")
        ...
        # 双 Agent 复核（监督网关的一种实现，非 Rule of Two 本身）：两个都通过才能执行
    ...
    def request_human_approval(self, cost, reason):
        """大额采购需要人类确认"""
        ...
        # 等待人类审批；获批后实际系统应重新进入 execute_purchase 执行流程（此处省略）
...
class SafeCodeReviewAgent:
    ...
    def review_and_merge(self, pull_request):
        ...
        # Step 4: 双 Agent 复核检查（监督网关，非 Rule of Two 本身）
...
1. LLM 应用框架
   - LangChain: 链式调用管理
   - LangGraph / AutoGen / CrewAI: 多智能体编排与工作流
   - LlamaIndex: RAG 框架
