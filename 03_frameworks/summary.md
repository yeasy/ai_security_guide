...
**OWASP LLM Top 10**：业界广泛采用的 LLM 安全风险清单，覆盖提示注入、敏感信息泄露、过度自主权、供应链风险、数据与模型投毒、无边界消耗、错误信息、隐藏上下文暴露、向量与嵌入弱点、输出处理不当等核心风险类别；具体条目与措辞请以 OWASP 官方最新发布为准。OWASP 另有一份**并行而非替代**的 *Top 10 For Agentic Applications 2026*（`ASI01`–`ASI10`），面向智能体应用，对照表见第七章章首。
...
**MITRE ATLAS**：以攻击者视角组织的 AI 对抗战术与技术（TTP）矩阵，仿照 ATT&CK，并新增 AI Model Access、AI Attack Staging 等 AI 专属战术；它把 OWASP/NIST 的“风险”翻译成可演练、可检测的具体攻击技术，是“风险—治理—对抗”三视角中的对抗一环。
...
| 场景 | 推荐框架 | 应用重点 |
|------|----------|----------|
| 风险识别 | OWASP LLM Top 10（2026） | 系统性检查十大风险 |
| 风险管理 | NIST AI RMF | 建立治理-映射-测量-管理流程 |
| 行业合规 | 行业标准 | 满足特定监管要求 |
| 对抗演练 / 红队 | MITRE ATLAS | 把风险翻译成可演练、可检测的攻击 TTP |
