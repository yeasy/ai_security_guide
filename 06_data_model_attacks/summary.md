...
**恶意模型工件与反序列化攻击**：模型文件本身可能是可执行载荷——pickle 格式（`.pkl`/`.pt`/`.bin`）在加载时执行代码，攻击者借此实现任意代码执行（如 nullifAI 绕过 Picklescan）。应优先使用 Safetensors，不要只依赖扫描器，并在沙箱中加载不可信模型。
...
| 恶意模型工件 | 加载阶段 | 任意代码执行（RCE） | 中等（仅依赖扫描器则高） |
...
| 恶意模型工件 | 优先 Safetensors、来源签名校验、沙箱加载、不只依赖 pickle 扫描器 |
