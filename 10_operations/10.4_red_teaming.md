...
```mermaid
flowchart TB
    A["制定 ROE<br/>明确范围与约束"] --> B["威胁建模<br/>识别高风险路径"]
    B --> C["构建攻击用例库"]
    C --> D["手动探测<br/>专家级攻击"]
    C --> E["自动化扫描<br/>工具批量探测"]
    D --> F["记录发现<br/>分级定责"]
...
| 工具 | 定位 | 适用场景 | 开源协议 |
|------|------|----------|----------|
| Garak | LLM 漏洞扫描器 | 已知漏洞模式批量探测 | Apache 2.0 |
| promptfoo | Prompt 评估框架 | 安全策略回归测试 | MIT |
| PyRIT | 微软红队工具 | 多轮自动化红队 | MIT |
| ART | IBM 对抗鲁棒性工具箱 | 对抗样本生成与评估 | MIT |
