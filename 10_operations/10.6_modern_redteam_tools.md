...
# 伪代码：DeepTeam 评估
evaluator = Evaluator(model='YOUR_EVALUATOR_MODEL')
...
| 类别 | 示例用途 |
|------|----------|
| GCMA | Jailbreak、DAN 等经典越狱 |
| Misleading | 误导性信息、错误事实 |
| Offensive | 冒犯性内容、仇恨言论 |
| Imitative | 模仿权威、身份欺骗 |
| Knownbads | 已知有害模式库 |
...
```python
# 伪代码：Garak 中可通过 generators 模块按配置生成新的攻击样本
generator = configure_generator(
    model='gpt-3.5-turbo',
    strategy='evolutionary'
)

new_attacks = generator.run(count=100)
```
...
前面的 `10.6.1` 到 `10.6.6` 重点在“工具能力与集成方式”。从这里开始，视角切换到企业落地中的流程和门禁设计：如何把这些工具接入 CI/CD、如何设置 ASR 阈值、如何组织团队。
