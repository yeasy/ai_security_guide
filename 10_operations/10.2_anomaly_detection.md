...
> 说明：若阅读环境不支持 Mermaid 渲染，可将 ML 异常检测理解为：从行为数据提取特征→模型输出异常评分→结合阈值与策略触发告警。
...
> 说明：若阅读环境不支持 Mermaid 渲染，可将升级机制理解为：告警触发后按严重级别分派与升级，直到被响应并关闭。
