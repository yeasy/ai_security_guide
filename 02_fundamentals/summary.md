...
**推理模型安全**：以 OpenAI `o` 系列、Claude thinking 模式和 DeepSeek-R1 为代表的推理模型通过额外推理预算提升复杂任务表现，但也引入了思维注入、推理链破坏、预算耗尽，以及 reasoning 暴露策略差异带来的新风险。开放权重模型的本地部署更面临模板替换、微调绕过和安全包装被拆除的根本困境，需要技术与治理的组合防御。
