...
- **Anthropic**：公开 API 自 Claude Opus 4.7 起改用 `thinking: {type: "adaptive"}` 搭配 `effort` 控制思考深度——**Opus 4.7、Opus 4.8、Opus 5、Sonnet 5、Fable 5 / Mythos 5 以及 2026-09-01 发布的 Fable 5.1 / Mythos 5.1 都不再接受手动模式**，继续传 `thinking: {type: "enabled"}` 会直接返回 400。Claude Opus/Sonnet 4.6 仍可使用 `thinking` + `budget_tokens`，但该手动模式已弃用；Sonnet 4.5、Opus 4.5、Haiku 4.5 及更早的 Claude 4 系列则只有手动模式可用，传 `adaptive` 同样报错。**这条分界对安全设计的意义是：思考预算不再是调用方能钉死的量**，自适应模式下模型自行决定是否思考、思考多久，低 effort 时甚至可能完全跳过，因此任何“每次请求都会产生可审计推理链”的假设都不成立。Claude 4 系列可能返回摘要、加密、`omitted` 或 `redacted_thinking` blocks，而不是稳定暴露原始推理链。2026-06 发布的 Claude Fable 5 / Mythos 5 更进一步，原始思维链永不返回，`thinking.display` 默认 `omitted`、可选 `summarized`；两者曾于 2026-06-12 被短暂暂停访问（官方模型页已恢复 Fable 5 为正常提供、Mythos 5 为受限可用），其接口设计从产品层面印证了“不要假设能稳定拿到原始推理链”的安全判断。
...
| **代表方法** | 经典提示注入、少样本注入 | TAP 变体、M2S 多步骤、STAR 结构化 |
...
推理模型代表了 AI 能力的重大进步，但也需要新的安全思维和方法。通过深入理解推理过程中的风险，并采用针对性的防御措施，组织可以安全地获得推理模型的强大能力。
