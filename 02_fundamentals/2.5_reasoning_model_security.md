...
- **Anthropic**：公开 API 对 Claude Opus 4.7/4.8 改用 `thinking: {type: "adaptive"}` 和 `effort` 控制扩展思考；Claude Opus/Sonnet 4.6 仍可使用 `thinking` + `budget_tokens`，但该手动模式已弃用。Claude 4 系列可能返回摘要、加密、`omitted` 或 `redacted_thinking` blocks，而不是稳定暴露原始推理链。2026-06 发布的 Claude Fable 5 / Mythos 5 进一步只支持 adaptive thinking，原始思维链永不返回，`thinking.display` 默认 `omitted`、可选 `summarized`；两者曾于 2026-06-12 被短暂暂停访问（官方模型页已恢复 Fable 5 为正常提供、Mythos 5 为受限可用），其接口设计从产品层面印证了“不要假设能稳定拿到原始推理链”的安全判断。
