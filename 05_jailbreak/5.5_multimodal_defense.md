...
随着多模态LLM的快速演进，多模态攻击的复杂性和隐蔽性也在不断提升。单一模态的防御策略已难以应对跨模态的协同攻击。本节系统阐述多模态安全防御的核心原则、技术方案和落地实践。
...
多模态系统防御点：
→ 图像输入验证 → 文字OCR检查 → 音频转录检查 → 融合层检验 → 跨模态一致性检查 → 输出审核 = 6+个防御层
```
...
def detect_hidden_audio_commands(audio_file):
    ...
    # 检查极低频频段（<20Hz）
    infrasonic_energy = np.sum(spectrogram[:2, :])

    # 检查异常频谱分布
    freq_anomaly = detect_unusual_frequency_patterns(spectrogram)

    risk_score = (ultrasonic_energy + infrasonic_energy + freq_anomaly) / 3
    return risk_score
```
...
def monitor_fusion_layer_activation(hidden_states, threshold=2.0):
    ...
    if np.sum(anomalies) > 0.1 * len(hidden_states):  # >10%的异常激活
...
传统的RLHF对齐主要针对文本模态。对于多模态模型，需要扩展安全对齐：
...
1. **对抗图像样本**
   - 使用PGD（Projected Gradient Descent）生成对抗图像
...
def generate_adversarial_multimodal_samples():
    ...
    # 3. 使用这些样本进行安全RLHF
    train_with_safety_rlhf(adversarial_dataset)
```
